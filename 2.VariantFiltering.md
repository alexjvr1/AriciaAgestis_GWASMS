...
vcftools --vcf Aagestis.withmissing.vcf --minQ 100 --min-meanDP 10 --max-meanDP 255 --max-alleles 2 --max-missing 0.3 --recode --recode-INFO-all --out AAgestis_FINAL
...
Outputting VCF file...
After filtering, kept 80456 out of a possible 558840 Sites
...
[Fig1]:https://user-images.githubusercontent.com/12142475/109155051-0722f380-7767-11eb-9f3c-cc2c3a8cc57c.png
...
FOR_25_2014_R1_.fastq.gz
FOR_26_2014_R1_.fastq.gz
FOR_30_2014_R1_.fastq.gz
...
LYD_31_2014_R1_.fastq.gz
LYD_34_2014_R1_.fastq.gz
MOF_42_2014_R1_.fastq.gz
SWD_17_2013_R1_.fastq.gz
SWD_7_2014_R1_.fastq.gz
```
...
```
vcftools --vcf AAgestis_FINAL.recode.vcf --remove indivstoremove --recode --recode-INFO-all --out AAgestis.266_FINAL
...
Parameters as interpreted:
	...
	--recode-INFO-all
	--out AAgestis.266_FINAL
	--recode

Excluding individuals in 'exclude' list
After filtering, kept 266 out of 286 Individuals
Outputting VCF file...
After filtering, kept 80456 out of a possible 80456 Sites
Run Time = 57.00 seconds


And check if the filters remove any more loci:

vcftools --vcf AAgestis.266_FINAL.recode.vcf --minQ 20 --min-meanDP 6 --max-missing 0.3 --recode --recode-INFO-all --out AAgestis.266_FINAL2

VCFtools - v0.1.12b
(C) Adam Auton and Anthony Marcketta 2009

Parameters as interpreted:
	--vcf AAgestis.266_FINAL.recode.vcf
	--recode-INFO-all
	--min-meanDP 6
	--minQ 20
	--max-missing 0.3
	--out AAgestis.266_FINAL2
	--recode

After filtering, kept 266 out of 266 Individuals
Outputting VCF file...
After filtering, kept 80456 out of a possible 80456 Sites
...
bcftools query -l AAgestis.266_FINAL2.recode.vcf > A266.oldnames

sed 's/_R1_.fastq.gz//g' A266.oldnames > A266.newnames

bcftools reheader AAgestis.266_FINAL2.recode.vcf -s A266.newnames -o AAgestis.266_FINAL2.newnames.vcf
```


Assess the data
These stats are based on the dataset before filtering for MAF 1% (80456 loci)
