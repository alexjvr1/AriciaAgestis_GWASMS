...
vcftools --vcf Aagestis.withmissing.vcf --minQ 20 --min-meanDP 6 --max-alleles 2 --max-missing 0.3 --recode --recode-INFO-all --out AAgestis_FINAL
...
Outputting VCF file...
After filtering, kept 135140 out of a possible 558840 Sites
Run Time = 131.00 seconds
...
vcftools --vcf Aagestis.withmissing.vcf --minQ 20 --min-meanDP 6 --max-meanDP 500 --max-alleles 2 --max-missing 0.3 --recode --recode-INFO-all --out AAgestis_FINAL
...
After filtering, kept 286 out of 286 Individuals
Outputting VCF file...
After filtering, kept 122809 out of a possible 558840 Sites
Run Time = 123.00 seconds
...
```
vcftools --vcf AAgestis_FINAL.recode.vcf --remove indivstoremove --recode --recode-INFO-all --out AAgestis.272_FINAL
...
Parameters as interpreted:
	...
	--recode-INFO-all
	--out AAgestis.272_FINAL
	--recode

Excluding individuals in 'exclude' list
After filtering, kept 272 out of 286 Individuals
Outputting VCF file...
After filtering, kept 122809 out of a possible 122809 Sites
Run Time = 79.00 seconds


#And check if the filters remove any more loci:

vcftools --vcf AAgestis.272_FINAL.recode.vcf --minQ 20 --min-meanDP 6 --max-missing 0.3 --recode --recode-INFO-all --out AAgestis.272_FINAL.recode.vcf

VCFtools - v0.1.12b
(C) Adam Auton and Anthony Marcketta 2009

Parameters as interpreted:
	--vcf AAgestis.272_FINAL.recode.vcf
	--recode-INFO-all
	--min-meanDP 6
	--minQ 20
	--max-missing 0.3
	--out AAgestis.272_FINAL.recode.vcf
	--recode

After filtering, kept 272 out of 272 Individuals
Outputting VCF file...
After filtering, kept 122729 out of a possible 122809 Sites
...
bcftools query -l AAgestis.272_FINAL.recode.vcf > A272.oldnames

sed 's/_R1_.fastq.gz//g' A272.oldnames > A272.newnames

bcftools reheader AAgestis.272_FINAL.recode.vcf -s A272.newnames -o AAgestis.272_FINAL.newnames.vcf
```


Assess the data
These stats are based on the dataset before filtering for MAF 1% (122729 loci)
...
Plot of mean depth per individual grouped by population: raw data first, and then filtered
...
There's a worrying correlation between MEAN_DEPTH and number of variants called in an individual. However, the mean depth is really high (>50x), and I've filtered for minQ (variant quality) PHRED 100. However, I can't filter individual genotypes without GQ (Genotype Quality) flag in the vcf file.
...
Before continuing with these analyses I will filter the dataset to include only loci genotyped in at least 50% of individuals in each population. I'm also filtering for MAF 1% as these loci are more likely to include errors introduced during PCR or sequencing.
...
bgzip all of these (this was done on the mac) and find the intersection using bcftools. This keeps only loci that are genotyped in all of the files. Various options available. See here
...
Estimate relatedness using the KING method implemented in vcftools.
...
Figure1 from Manichaikul et al. 2010 shows that there us a lot of variance in the expected measure of relatedness when using only a few markers. As this method assumes unlinked markers, the thinned dataset includes only 3831 variants.

alt_txt

Given the small number of markers I can only accurately identify first and second order relatives. Third order relatives can't be distinguished from unrelated individuals. Using a (second order relative) cut-off of 0.05 I identified a few related individuals, mostly within the WIS pop:
...
And filter to keep only loci genotyped in 50% of individuals across all species: First split the vcf file by population
